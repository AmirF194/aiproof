...
This file is the research layer behind [README.md](README.md) and [REPORT.md](REPORT.md). Every number here is sourced. Use it to pressure-test the scores in [data/roles.csv](data/roles.csv).

The headline: **the labor market for software has already split into three economies — and the split is wider than the public narrative suggests.** This file walks through the evidence cluster by cluster.
...
UI Designer (40) and Product Designer (64) hold their tier; **UX Researcher** is the under-promoted role that the 36-role list groups under "UX Designer / Researcher" — if split, the researcher half would land in the high-Safe range (~75–78).
...
- The numbers in the role tables ([data/roles.csv](data/roles.csv)) are the author's calibrated estimates based on the data above.
- The *value* of this analysis is the **rubric consistency**, not the precision of any individual score. If you disagree with a specific score, re-weight in `data/roles.csv` and regenerate via `python3 scripts/generate_charts.py`.
