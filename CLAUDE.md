...
- **`Role.last_updated` is the roster clock, not the data clock.** It is `auto_now` on `Role`, so it moves when `load_roles` runs and never when the weekly crawl runs. Use `RoleMetric.updated_at` for anything that means "when was the live data last refreshed". The homepage advertised 2026-05-18 for three months because it read the wrong one.
